...
Go 版の通知 Lambda（lambda_go/deploy-notifier）と合わせて、
このリポジトリの実行系ログを 1 行 JSON に揃えるためのユーティリティ。

ECS Fargate では標準出力が awslogs ドライバ経由でそのまま CloudWatch Logs に入るため、
sink は既定の標準出力のままで Logs Insights から検索・集計できる。
